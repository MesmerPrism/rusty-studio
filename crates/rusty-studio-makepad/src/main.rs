...
impl App {
    ...
    fn review_shell_hostess_staging_file_plan(&mut self, cx: &mut Cx) {
        let Some(source) = self.project_source.clone() else {
            self.last_shell_bundle_status = "No project source is loaded".to_string();
            self.sync_loaded_model(cx);
            self.ui.redraw(cx);
    ...
    fn review_shell_hostess_staging_handoff(&mut self, cx: &mut Cx) {
        let Some(source) = self.project_source.clone() else {
            self.last_shell_bundle_status = "No project source is loaded".to_string();
            self.sync_loaded_model(cx);
            self.ui.redraw(cx);
    ...
    fn review_shell_hostess_staging_acceptance(&mut self, cx: &mut Cx) {
        let Some(source) = self.project_source.clone() else {
            self.last_shell_bundle_status = "No project source is loaded".to_string();
            self.sync_loaded_model(cx);
            self.ui.redraw(cx);
...
fn shell_hostess_staging_file_plan_for_project_source(
    project_path: &Path,
...
fn shell_hostess_staging_handoff_for_project_source(
    project_path: &Path,
...
fn shell_hostess_staging_acceptance_for_project_source(
    project_path: &Path,
...
fn shell_hostess_staging_file_plan_output_path(project_path: &Path) -> PathBuf {
    project_path
        .parent()
        .unwrap_or_else(|| Path::new("."))
        .join("target")
...
fn shell_hostess_staging_handoff_output_path(project_path: &Path) -> PathBuf {
    project_path
        .parent()
        .unwrap_or_else(|| Path::new("."))
        .join("target")
...
fn shell_hostess_staging_acceptance_output_path(project_path: &Path) -> PathBuf {
    project_path
        .parent()
        .unwrap_or_else(|| Path::new("."))
        .join("target")
...
fn shell_hostess_staging_file_plan_status(
...
fn shell_hostess_staging_handoff_status(
...
fn shell_hostess_staging_acceptance_status(
...
fn shell_hostess_staging_file_plan_status_label(
...
fn shell_hostess_staging_file_request_status_label(
...
fn shell_hostess_staging_handoff_status_label(
...
fn shell_hostess_staging_handoff_instruction_status_label(
...
fn shell_hostess_staging_acceptance_status_label(
  ...
  "evidence_pull_route": "evidence.adb_pull"
    ...
    fn shell_hostess_handoff_package_reports_ready_from_makepad_route() {
        ...
        let (handoff, handoff_path) =
            shell_hostess_staging_handoff_for_project_source(&project_path)
        ...
        let acceptance_status =
            shell_hostess_staging_acceptance_status(&acceptance, &acceptance_path);
