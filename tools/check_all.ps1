...
. (Join-Path $PSScriptRoot "checks\studio_hostess_staging_check.ps1")
. (Join-Path $PSScriptRoot "checks\studio_shell_export_package_check.ps1")
...
try {
    ...
    $ShellHostessStagingHandoffPath = Join-Path $RepoRoot "target\studio-shell-handoffs\shell-hostess-staging-handoff.json"
    $ShellHostessStagingAcceptancePath = Join-Path $RepoRoot "target\studio-shell-handoffs\shell-hostess-staging-acceptance-checklist.json"
    $ShellHostessStagingAcceptanceManifestPath = Join-Path $RepoRoot "target\studio-shell-handoffs\shell-hostess-staging-acceptance-manifest.json"
    $ShellHostessStagingAcceptanceArchiveManifestPath = Join-Path $RepoRoot "target\studio-shell-handoffs\shell-hostess-staging-acceptance-manifest-archive.json"
    $ShellHostessStagingAcceptanceIndexPath = Join-Path $RepoRoot "target\studio-shell-handoffs\shell-hostess-staging-acceptances.json"
    ...
    $ShellExportPackageCheckArgs = @{
        DamagedShellBundleRoot = $DamagedShellBundleRoot
        DamagedShellExportPackagePath = $DamagedShellExportPackagePath
        DamagedShellHandoffManifestPath = $DamagedShellHandoffManifestPath
        DamagedTemplateShellBundleRoot = $DamagedTemplateShellBundleRoot
        DamagedTemplateShellExportPackageBaselinePath = $DamagedTemplateShellExportPackageBaselinePath
        DamagedTemplateShellExportPackagePath = $DamagedTemplateShellExportPackagePath
        DamagedTemplateShellHandoffManifestPath = $DamagedTemplateShellHandoffManifestPath
        HandoffManifest = $HandoffManifest
        RegressedShellExportPackageComparisonPath = $RegressedShellExportPackageComparisonPath
        SelectedShellBundleRoot = $SelectedShellBundleRoot
        ShellExportPackageBaselineIndexPath = $ShellExportPackageBaselineIndexPath
        ShellExportPackageBaselinePath = $ShellExportPackageBaselinePath
        ShellExportPackageBaselineSelectionPath = $ShellExportPackageBaselineSelectionPath
        ShellExportPackageComparisonPath = $ShellExportPackageComparisonPath
        ShellExportPackageIndexComparisonPath = $ShellExportPackageIndexComparisonPath
        ShellExportPackageMultiBaselineIndexPath = $ShellExportPackageMultiBaselineIndexPath
        ShellExportPackagePath = $ShellExportPackagePath
        ShellExportPackagePromotedBaselineIndexPath = $ShellExportPackagePromotedBaselineIndexPath
        SourceProjectPath = $SourceProjectPath
    }
    Invoke-StudioShellExportPackageCheck @ShellExportPackageCheckArgs
    ...
    $HostessStagingCheckArgs = @{
        ProjectedMotionShellHandoffEvidencePath = $ProjectedMotionShellHandoffEvidencePath
        ProjectedMotionShellHandoffReviewPath = $ProjectedMotionShellHandoffReviewPath
        ShellHandoffManifestPath = $ShellHandoffManifestPath
        ShellHostessHandoffPackagePath = $ShellHostessHandoffPackagePath
        ShellHostessOwnerIntakePath = $ShellHostessOwnerIntakePath
        ShellHostessStagingAcceptanceArchiveManifestPath = $ShellHostessStagingAcceptanceArchiveManifestPath
        ShellHostessStagingAcceptanceComparisonPath = $ShellHostessStagingAcceptanceComparisonPath
        ShellHostessStagingAcceptanceIndexPath = $ShellHostessStagingAcceptanceIndexPath
        ShellHostessStagingAcceptanceManifestPath = $ShellHostessStagingAcceptanceManifestPath
        ShellHostessStagingAcceptanceMultiIndexPath = $ShellHostessStagingAcceptanceMultiIndexPath
        ShellHostessStagingAcceptancePath = $ShellHostessStagingAcceptancePath
        ShellHostessStagingAcceptancePromotedIndexPath = $ShellHostessStagingAcceptancePromotedIndexPath
        ShellHostessStagingAcceptanceRouteDriftComparisonPath = $ShellHostessStagingAcceptanceRouteDriftComparisonPath
        ShellHostessStagingAcceptanceRouteDriftPath = $ShellHostessStagingAcceptanceRouteDriftPath
        ShellHostessStagingAcceptanceSelectionPath = $ShellHostessStagingAcceptanceSelectionPath
        ShellHostessStagingExecutionRequestPath = $ShellHostessStagingExecutionRequestPath
        ShellHostessStagingFilePlanPath = $ShellHostessStagingFilePlanPath
        ShellHostessStagingHandoffPath = $ShellHostessStagingHandoffPath
        ShellHostessStagingPreviewPath = $ShellHostessStagingPreviewPath
        ShellReleaseCandidateReviewManifestPath = $ShellReleaseCandidateReviewManifestPath
        ShellReleaseCandidateReviewPath = $ShellReleaseCandidateReviewPath
        ShellReleaseCandidateReviewPromotedIndexPath = $ShellReleaseCandidateReviewPromotedIndexPath
    }
    Invoke-StudioHostessStagingCheck @HostessStagingCheckArgs
    ...
    $SyntheticProjectStatus = & git status --short -- $SourceProjectPath
    if ($LASTEXITCODE -eq 0 -and ($SyntheticProjectStatus -join "").Trim().Length -gt 0) {
        throw "tracked synthetic Studio project fixture was modified during checks"
    }
} finally {
    Pop-Location
}
